...
# Copilot release: test -> build release-signed APK -> publish as the latest
...
if [ ! -f "keystore/signing.properties" ] || [ ! -f "keystore/copilot-release.jks" ]; then
  die "in-repo keystore missing (keystore/signing.properties + keystore/copilot-release.jks)"
fi
...
OUT_APK="copilot-$VERSION.apk"
cp "$SRC_APK" "$OUT_APK"

# --- push, tag + publish ---
echo "==> push branch + tag + GitHub release"
git push origin HEAD
git tag -a "$TAG" -m "Copilot $VERSION"
git push origin "$TAG"
gh release create "$TAG" \
  --repo "$REPO" \
  --latest \
  --title "Copilot $VERSION" \
  --generate-notes \
  "$OUT_APK"
